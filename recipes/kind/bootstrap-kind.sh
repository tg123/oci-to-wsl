...
# in place. The script is idempotent and safe to re-run.
set -euxo pipefail
...
# If the control plane is already up (re-run), nothing more to do.
if [ -f /etc/kubernetes/admin.conf ] && \
   KUBECONFIG=/etc/kubernetes/admin.conf kubectl get nodes >/dev/null 2>&1; then
  echo "control plane already initialised; skipping kubeadm init"
  KUBECONFIG=/etc/kubernetes/admin.conf kubectl wait --for=condition=Ready nodes --all --timeout=300s
  exit 0
fi
